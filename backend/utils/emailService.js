...
if (process.env.EMAIL_USER && process.env.EMAIL_PASSWORD) {
  ...
  console.warn("⚠️ Email transporter not configured: EMAIL_USER or EMAIL_PASSWORD missing in env.");
}
...
export const sendEmail = async (options) => {
  try {
    ...
    if (!process.env.EMAIL_USER || !process.env.EMAIL_PASSWORD) {
      console.warn(`⚠️ Email skipped (SMTP not configured) for: ${email} (Subject: "${subject}")`);
    ...
    // Do not throw an error that crashes the calling transaction/API request; log and proceed.
