...
const BloodCamps = () => {
  ...
  const calculateStats = (campsData) => {
    ...
    const stats = {
      upcoming: campsData.filter(
        (camp) => String(camp.status).toLowerCase() === "upcoming",
      ).length,
      ongoing: campsData.filter(
        (camp) => String(camp.status).toLowerCase() === "ongoing",
      ).length,
      completed: campsData.filter(
        (camp) => String(camp.status).toLowerCase() === "completed",
      ).length,
      cancelled: campsData.filter(
        (camp) => String(camp.status).toLowerCase() === "cancelled",
      ).length,
  ...
  const fetchCamps = useCallback(
    async (page = 1, { silent = false } = {}) => {
      try {
        ...
        console.log("📡 Fetching camps with params:", params);

        const res = await api.get("/blood-lab/camps", { params });
        const data = res.data;
  ...
  const updateCampStatus = async (campId, newStatus) => {
    try {
      ...
      const attempts = [
        () => bloodLabApi.updateCampStatus(campId, { status: newStatus }),
        () => bloodLabApi.updateCamp(campId, { status: newStatus }),
      ];
      ...
      for (const attempt of attempts) {
        try {
          ...
          console.warn("⚠️ Status update attempt failed:", err.message);
          lastError = err;
        }
      }
  ...
  useCampRealtime(() => {
    fetchCamps(pagination.currentPage, { silent: true });
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = editingCamp
        ? await bloodLabApi.updateCamp(editingCamp._id, payload)
        : await bloodLabApi.createCamp(payload);
      const data = res.data;
  ...
  const getAvailableActions = (camp) => {
    ...
    switch (String(camp.status).toLowerCase()) {
      case "upcoming":
        baseActions.push(
