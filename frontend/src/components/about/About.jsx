...
// Create separate FAQItem component
  ...
  const values = [
    {
      ...
      title: "Compassion",
      description:
        "Every drop tells a story of hope. We treat every donor and recipient with the empathy and dignity they deserve, because behind every blood bag is a life waiting to be saved.",
      color: "red",
    },
    {
      icon: Shield,
      title: "Safety First",
      description:
        "Safety is our non-negotiable promise. Every donation follows rigorous medical protocols with sterile equipment and multiple quality checks, ensuring zero compromise on donor well-being or blood purity.",
      color: "blue",
    },
    {
      icon: Users,
      title: "Community",
      description:
        "We believe in the power of united action. By bringing together donors, hospitals, and volunteers, we create a resilient network where help is always just a heartbeat away.",
      color: "green",
    },
    {
      icon: Target,
      title: "Excellence",
      description:
        "Good intentions meet exceptional execution. From seamless donor experience to instant emergency response, we continuously raise the bar because the lives we touch deserve nothing less than our best.",
      color: "purple",
    },
  ];
      ...
      <section className="py-20 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            <span className="text-red-600 font-semibold text-sm tracking-wider uppercase mb-2 block">
              What We Stand For
            </span>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
            {values.map((value, index) => {
              ...
              return (
                <div
                  key={index}
                  className="group relative bg-white rounded-2xl p-7 shadow-lg hover:shadow-2xl transition-all duration-500 overflow-hidden"
                >
                  {/* Animated gradient background on hover */}
                  <div
                    className={`absolute inset-0 bg-gradient-to-br ${colors.bg} opacity-0 group-hover:opacity-20 transition-opacity duration-500`}
                  ></div>

                  {/* Icon with ring effect */}
                  <div className="relative mb-5">
                    <div
                      className={`${colors.bg} w-16 h-16 rounded-full flex items-center justify-center mx-auto ring-4 ring-white shadow-lg`}
                    >
                      <value.icon className={`w-8 h-8 ${colors.text}`} />
                    </div>

                    {/* Floating particles effect */}
                    <div className="absolute -top-1 -right-1 w-3 h-3 bg-red-400 rounded-full animate-ping opacity-0 group-hover:opacity-100"></div>
                    <div className="absolute -bottom-1 -left-1 w-2 h-2 bg-red-300 rounded-full animate-ping delay-100 opacity-0 group-hover:opacity-100"></div>
                  </div>

                  {/* Title with gradient text on hover */}
                  <h3
                    className={`text-xl font-bold text-center mb-3 group-hover:bg-gradient-to-r ${colors.text} group-hover:from-red-600 group-hover:to-red-400 group-hover:text-transparent group-hover:bg-clip-text transition-all duration-300`}
                  >
                    {value.title}
                  </h3>
                  ...
                  {/* Bottom accent bar */}
                  <div
                    className={`absolute bottom-0 left-1/2 transform -translate-x-1/2 w-0 h-1 ${colors.bg} group-hover:w-full transition-all duration-500 rounded-t-lg`}
                  ></div>
                </div>
              );
            })}
